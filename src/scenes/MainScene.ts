  ...
  private draw(time: number) {
    ...
    if (isControl) {
      ...
      this.commManager.getActivePolls().forEach(poll => {
        ...
        // A. Call Streamline (Hub -> Target): 伝達可能距離で打ち切る
        if (!poll.callReached) {
          const callMaxRange = poll.rangeMode === 'long' ? 2500 : 750;
          const t = Math.min(1.0, Math.min(waveDist, callMaxRange) / poll.distance);
